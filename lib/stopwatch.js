...
class StopWatch extends Component {
  ...
  start() {
    if (!this.state.started) {
      if (this.props.laps && this.elapsed) {
      ...
      this.setState({
        startTime: this.elapsed ? new Date() - this.elapsed :
          new Date(), started: true
      });

      this.interval = this.interval ? this.interval : setInterval(() => {
        this.elapsed = new Date() - this.state.startTime - this.state.pausedTime;
        this.formatTime();
      }, 1000);
    }
  }
  ...
  reset() {
    this.elapsed = null;
    this.setState({ startTime: null, stopTime: null, pausedTime: null});
  }

  formatTime() {
    let now = this.elapsed;
    let msecs = now % 1000;
